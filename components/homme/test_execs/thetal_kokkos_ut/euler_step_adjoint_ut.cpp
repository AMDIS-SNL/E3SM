...
// selection. There is currently no Sacado/Fad ETI plumbed through far
// enough (Tracers/ElementsDerivedState/BoundaryExchange all need it) to
// get an "exact" tangent via automatic differentiation the way
// imex_adjoint_ut.cpp/prim_advance_sacado_ut.cpp do for CAAR/DIRK, so both
// checks below are central-difference finite-difference checks (small
// step, so -- away from a limiter/min-max kink -- the map is locally exact
// affine and the FD ratio should match the adjoint's prediction to close
// to roundoff; tolerances are chosen accordingly). A future pass wiring up
// EulerStepFunctorImplST<DpFadType> (already ETI'd behind
// HOMMEXX_ENABLE_FAD_TYPES) end to end would let this become a true
// dot-product-exact check, mirroring imex_adjoint_ut.cpp.
//
// Both sections below deliberately perturb qdp so that, at at least one
// GLL point, compute_qtens's raw (pre-limiter) output lands outside the
// local [minp,maxp] and the clip in limiter_clip_and_sum actually fires --
// a test that only exercised the smooth/unclipped interior would not catch
// a wrong clip-branch adjoint.
//
...
TEST_CASE("euler_step_adjoint")
{
  ...
  auto& sphops = c.create<SphereOperatorsST<ScalarValue>>();
  ...
  // EulerStepFunctor's (EulerStepFunctorST<ScalarValue>, ScalarValue==Real
  // in a normal, non-HOMMEXX_ENABLE_FWD_SENS build) default constructor
  // pulls ElementsGeometry/ElementsDerivedState/Tracers/ReferenceElement/
  // HybridVCoord/SphereOperatorsST straight out of Context, so all of the
  // above must be registered first.
  EulerStepFunctor euler;
  euler.reset(params);
  ...
  Tracers adj_tracers; adj_tracers.init(num_elems,qsize);
  ElementsDerivedState adj_derived; adj_derived.init(num_elems);
  ...
  auto run_one_trial = [&] (const Real eps) -> std::pair<double,double> {
    ...
    Tracers dJ_dqdp_seed; dJ_dqdp_seed.init(num_elems,qsize);
    genRandArray(dJ_dqdp_seed.qdp, engine, RPDF(-1.0,1.0));
    ElementsDerivedState dJ_deta_seed; dJ_deta_seed.init(num_elems);
